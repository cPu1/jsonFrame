...
JsonRpcClient.prototype.buildRequest = function(method, params) {
	...
	if(params != null && !Array.isArray(params)) throw Error('Params must be absent or an array of parameters');
	...
	if (isBatch) {
		...
		batchIds.push.apply(batchIds, batchBuilder._batchIds);
...
*/
JsonRpcClient.prototype.batchBuilder = {
	_batch: [],
	_batchIds: [],
	add: function (method, params) {
		// console.log('me is', this.that.constructor)
 ...
 */
JsonRpcClient.prototype.buildResponse = function(rpcResponse) {
	...
	// console.log('Handling ', rpcResponse, fnQueue);
	...
	if (isBatch) {
		...
		// console.log('Sorted responses', rpcResponse);
		/**
...
JsonRpcClient.prototype._handleResponse = function(rpcResponse) {
...
/**
_ notification flag is internal and not part of jsonrpc 2.0
*/
JsonRpcClient.prototype._buildNotification = function(method, params) {
	// TODO: batch notifications
	if(typeof method !== 'string' && params == null && !Array.isArray(params))
